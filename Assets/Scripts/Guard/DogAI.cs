...
public class DogAI : NetworkBehaviour
{
    ...
    {
        if (chaseTarget == null || chaseTarget.IsEliminated || chaseTarget.IsHiding || chaseTarget.IsLockedUp) //gone, out, hidden, or already jailed by the guard - the dog drops them instead of barking the guard back onto someone he's already caught
        {
            ChangeState(DogState.Investigating, null);
            return;
        }
